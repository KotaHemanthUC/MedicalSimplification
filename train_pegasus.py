...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        source = self.tokenizer.batch_encode_plus(
            [ctext], max_length=self.source_len, truncation=True, padding=True, return_tensors="pt")
        target = self.tokenizer.batch_encode_plus(
            [text], max_length=self.summ_len, truncation=True, padding=True, return_tensors="pt")
...
def collate_fn(batch):
    ...
    target_ids_y = [item['target_ids_y'] for item in batch]

    # Pad sequences to the maximum length within each batch
    source_ids_padded = pad_sequence(source_ids, batch_first=True)
    source_mask_padded = pad_sequence(source_mask, batch_first=True)
    target_ids_padded = pad_sequence(target_ids, batch_first=True)
    target_ids_y_padded = pad_sequence(target_ids_y, batch_first=True)

    return {
        'source_ids': source_ids_padded,
        'source_mask': source_mask_padded,
        'target_ids': target_ids_padded,
        'target_ids_y': target_ids_y_padded
    }
...
def main():
    ...
    training_loader = DataLoader(
        training_set, collate_fn=collate_fn, **train_params)
    val_loader = DataLoader(val_set, collate_fn=collate_fn, **val_params)

    # Defining the model. We are using t5-base model and added a Language model layer on top for generation of Summary.
    # Further this model is sent to device (GPU/TPU) for using the hardware.
    # model = AutoModelForSeq2SeqLM.from_pretrained("google/flan-t5-base") #T5ForConditionalGeneration.from_pretrained("t5-base")
    ...
    for epoch in range(VAL_EPOCHS):
        ...
        final_df.to_csv('./predictions_pegasus.csv')
